    ...
    AND EXISTS (
      ...
      WHERE m_prev.`Инвентарный_номер` = m.`Инвентарный_номер`
        AND (m_prev.`Дата_перемещения`, m_prev.`ID_Перемещения`) =
          (
            SELECT m2.`Дата_перемещения`, m2.`ID_Перемещения`
            ...
            ORDER BY m2.`Дата_перемещения` DESC, m2.`ID_Перемещения` DESC
            LIMIT 1
          )
        AND s_prev.`Название_подразделения` = p_subdivision
    );
...
BEGIN
  ...
  START TRANSACTION;
    -- получить инвентарный номер из ремонта
    ...
    -- вычислить срок ремонта в днях
    SET v_days = DATEDIFF(p_date, v_move_date);

    IF v_days < 0 THEN
      SIGNAL SQLSTATE '45000'
        SET MESSAGE_TEXT = 'Дата окончания ремонта меньше даты приёма в ремонт';
    END IF;

    -- записать срок ремонта в таблицу Ремонт (формат: 'N дней')
    UPDATE `Ремонт`
    SET `Срок_ремонта` = CONCAT(v_days, ' дней')
    WHERE `ID_Ремонта` = p_repair_id;
    ...
    IF p_writeoff THEN
      ...
      -- при списании не вставляем перемещение (нет подразделения "Списание")
    ELSE
      -- найти предыдущее подразделение — откуда пришло в ремонт
      ...
      -- вставить перемещение обратно в предыдущее подразделение (устройство возвращается)
      INSERT INTO `Перемещение_устройства` (
        `Инвентарный_номер`,
        `Дата_перемещения`,
        `ID_Подразделения`
