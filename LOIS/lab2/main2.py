...
- Ковальчук Виктория Николаевна
- Семков Артур Андреевич
Вариант 2

Реализация обратного нечёткого логического выводв на основе операции нечёткой композиции {max({min({xi} ∪ {yi}) | i})}.
15.12.2025
...
def validate_indices(names_list, prefix):
    ...
    for name in names_list:
        if not name.startswith(prefix):
            print(f"Ошибка: Переменная '{name}' должна начинаться с '{prefix}'")
            sys.exit()
        
        # Извлекаем часть после префикса
        idx_str = name[len(prefix):]
        
        # Проверка, является ли остаток числом (без try-except)
        if not idx_str.isdigit():
            print(f"Ошибка: Переменная '{name}' не содержит корректного числового индекса")
            sys.exit()
    ...
    for i in range(len(indices) - 1):
        if indices[i] >= indices[i + 1]:
            print(
                f"Ошибка: Нарушен порядок индексов для {prefix}: "
...
def parse_input(filename):
    ...
    validate_indices(y_names, 'y')

    b_values = [float(x) for x in lines[1].split()]

    if len(y_names) != len(b_values):
        print(f"Ошибка: Количество переменных Y ({len(y_names)}) не совпадает с количеством значений ({len(b_values)})")
        sys.exit()

    # line 2: имена X (x1 x2)
    x_names = lines[2].split()
    validate_indices(x_names, 'x')
    num_x = len(x_names)

    # line 3+: матрица
    matrix = []
    for l_idx, l in enumerate(lines[3:]):
        row = [float(x) for x in l.split()]
            
        if len(row) != num_x:
            print("Ошибка: Неверное количество столбцов в матрице")
            sys.exit()
        matrix.append(row)

    if len(matrix) != len(b_values):
        print("Ошибка: Размер матрицы не совпадает с вектором B")
        sys.exit()
...
def write_results(filename, final_vectors, x_names):
    ...
    # Открытие файла без try-except. Если нет прав доступа, программа упадет сама.
