...
class SettingsView(IView):
    ...
    def handle_input(self):
        """
        Перенёс сюда привязку обработчиков ввода для элементов настроек:
        - callback ползунка (обновление метки)
        - команды кнопок Set и Tune
        Вызывается из App после render().
        """
        try:
